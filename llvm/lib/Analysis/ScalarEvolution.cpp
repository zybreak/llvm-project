...
const SCEV *SCEVUse::computeCanonical(ScalarEvolution &SE, const SCEV *S) {
  return SCEVDropFlags::rewrite(S, SE);
}
...
SCEVUse ScalarEvolution::getLosslessPtrToIntExpr(SCEVUse Op, unsigned Depth) {
  ...
  if (auto *U = dyn_cast<SCEVUnknown>(Op)) {
    ...
    if (Op.isCanonical())
      ...
      S->setCanonical(SCEVUse::computeCanonical(*this, S));
    return S;
  }
...
SCEVUse ScalarEvolution::getOrCreateAddExpr(ArrayRef<SCEVUse> Ops,
  ...
  if (!S) {
    ...
    UniqueSCEVs.InsertNode(S, IP);
    registerUser(S, Ops);
    if (all_of(Ops, [](SCEVUse U) { return U.isCanonical(); }))
      S->setCanonical(S);
    else
      S->setCanonical(SCEVUse::computeCanonical(*this, S));
  }
...
SCEVUse ScalarEvolution::getPredicatedExitCount(
...
static const SCEV *
SolveLinEquationWithOverflow(const APInt &A, SCEVUse B,
                             SmallVectorImpl<const SCEVPredicate *> *Predicates,
...
ScalarEvolution::ScalarEvolution(Function &F, TargetLibraryInfo &TLI,
  ...
  CouldNotCompute->setCanonical(&*CouldNotCompute);
