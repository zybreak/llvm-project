...
void RegBankLegalizeHelper::lower(MachineInstr &MI,
  ...
  case UniExtToSel: {
    ...
    auto True =
        B.buildConstant({SgprRB, Ty}, MI.getOpcode() == G_SEXT ? -1 : 1);
...
const RegisterBank *
RegBankLegalizeHelper::getRegBankFromID(RegBankLLTMapingApplyID ID) {
  switch (ID) {
  case Vcc:
    return VccRB;
  case Sgpr16:
...
void RegBankLegalizeHelper::applyMappingDst(
  ...
  for (; OpIdx < MethodIDs.size(); ++OpIdx) {
    ...
    case UniInVcc: {
      ...
      Op.setReg(NewDst);
      auto CopyS32_Vcc = B.buildInstr(G_COPY_SCC_VCC, {SgprRB_S32}, {NewDst});
  ...
  for (unsigned i = 0; i < MethodIDs.size(); ++OpIdx, ++i) {
    ...
    case Vcc: {
      ...
      if (RB == SgprRB) {
        ...
        auto CopyVcc_Scc = B.buildInstr(G_COPY_VCC_SCC, {VccRB_S1}, {Aext});
